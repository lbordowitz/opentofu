...
* On Windows, OpenTofu searches for configurations files in the following locations 
  and merges them together. Higher-precedence configurations overwrite the settings 
  of lower-precedence configurations. Configuration files are merged in the 
  following order:
  1. %APPDATA%/OpenTofu Project/OpenTofu/*.tfrc or *.tfrc.json
  1. %APPDATA%/terraform.d/*.tfrc or *.tfrc.json
  1. %APPDATA%/tofu.rc
  1. %APPDATA%/terraform.rc (for backward-compatibility purposes)
...
* On Unix systems, OpenTofu searches for configuration files in the following
  places and then merges them together. Higher-precedence configurations overwrite 
  the settings of lower-precedence configurations. Configuration files are merged 
  in the following precedence order:
  1. $XDG_CONFIG_HOME/opentofu/*.tfrc and *.tfrc.json
  1. for each $config in $XDG_CONFIG_DIRS, $config/opentofu/*.tfrc and *.tfrc.json, 
     with the earliest directory listed taking precedence 
  1. $HOME/.terraform.d for compatibility only
  1. $XDG_CONFIG_HOME/opentofu/tofurc
  1. for each $config in $XDG_CONFIG_DIRS, $config/opentofu/tofurc
  1. $HOME/.tofurc
  1. $HOME/.terraformrc
  If `$XDG_CONFIG_HOME` is not set, `$HOME/.config/opentofu` is used. Similarly, if 
  `$XDG_CONFIG_DIRS` is not set, `/etc/xdg` is used. With `$XDG_CONFIG_DIRS`, 
  earlier-listed directories take precedence. Note that this mostly follows the 
  [XDG Base Directory](https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html) specification, with some extra directories and files 
  added for the sake of compatibility.
* On macOS, OpenTofu searches for configurations files in the following places and merges them together. As with Unix, higher-precedence configurations overwrite the settings of lower-precedence configurations. Configuration files are merged in the following precedence order:
  1. ~/Library/Application Support/org.opentofu/*.tfrc and *.tfrc.json
  1. /Library/Application Support/org.opentofu/*.tfrc and *.tfrc.json
  1. For compatibility reasons, the directories and files in the Unix section, ordered as listed 
...
Any such file should follow the naming pattern `*.tfrc`. If this environment 
variable is set, all other configuration files in the aforementioned directories 
are ignored.
