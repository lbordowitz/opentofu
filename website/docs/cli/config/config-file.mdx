...
* On Windows, the file must be named `tofu.rc` and placed
  ...
  If both `terraform.rc` and `tofu.rc` files exists, the later would take 
  precedence. <TODO> Check that this is really how Windows acts, as we are changing 
  the rules a bit to use userdirs.ForApp instead of the custom windows code
