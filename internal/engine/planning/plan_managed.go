...
func (p *planGlue) planDesiredManagedResourceInstance(
	...
	if prevRoundState == nil {
		...
		if prevRoundState != nil && (resourceType.ResourceTypeName() != prevRoundState.ResourceType || !inst.Provider.Equals(prevRoundState.ProviderInstanceAddr.Config.Config.Provider)) {
			...
			*/
			...
			resp := providerClient.MoveResourceState(ctx, req)
			...
			movedPrevRoundState = movedPrevState
			movedPrevRoundPrivate = resp.TargetPrivate
		}

		if movedPrevRoundState != nil {
			obj, err := states.DecodeResourceInstanceObjectFull(movedPrevRoundState, schema.Block.ImpliedType())
			if err != nil {
				diags = diags.Append(tfdiags.AttributeValue(
					tfdiags.Error,
			...
			prevRoundPrivate = movedPrevRoundPrivate
			...
			// No move occurred; this is just a configured address without any state
	...
	if eq, _ := planResp.Planned.Value.Equals(refreshedVal).Unmark(); eq.IsKnown() && eq.True() && !forceReplace {
		...
		if !inst.Addr.Equal(prevRunAddr) {
			// A move occurred! Be sure to track this.
